...
import { feedback_insights } from '@/lib/db/schema';
  ...
  try {
    ...
    if (insightId) {
      const [row] = await db
        ...
        .where(eq(feedback_insights.id, insightId))
        .limit(1);
      insight = row;
    } else {
      const [row] = await db
    ...
    // 產出 Markdown 格式報告
    const issues = insight.issues as unknown as Array<{
      title: string;
      frequency: number;
      severity: string;
    ...
    const selfReview = insight.self_review as unknown as {
      what_worked: string;
      what_failed: string;
      improvement_plan: string;
    };
    const recommendations = insight.recommendations as unknown as Array<{
      priority: string;
      action: string;
...
### 自我檢討
- ✅ 做得好：${selfReview?.what_worked ?? 'N/A'}
- ❌ 待改善：${selfReview?.what_failed ?? 'N/A'}
- 📋 下期計畫：${selfReview?.improvement_plan ?? 'N/A'}
