...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!storeId) return NextResponse.redirect(new URL(`${BASE_PATH}/dashboard`, request.url))

    const session = await auth()
    if (!session?.user?.id) return NextResponse.redirect(new URL(`${BASE_PATH}/login`, request.url))
    ...
    if (!ownedStore) {
      ...
      if (!membership) return NextResponse.redirect(new URL(`${BASE_PATH}/dashboard`, request.url))
    }

    const returnTo = request.nextUrl.searchParams.get('returnTo')
    const dashboardUrl = returnTo && returnTo.startsWith(`${BASE_PATH}/dashboard`) ? returnTo : `${BASE_PATH}/dashboard?s=${Date.now()}`
