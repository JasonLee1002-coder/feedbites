...
export async function GET(
  ...
  try {
    ...
    // Verify survey belongs to this store
...
export async function POST(
  ...
  try {
    ...
    const [storeRow] = await db
      ...
      .from(stores)
      .where(eq(stores.id, survey.store_id))
      .limit(1)

    if (storeRow?.owner_line_user_id) {
      const storeLineId = storeRow.owner_line_user_id
      const storeName = storeRow.store_name
      const hourAgo = new Date(Date.now() - 3600000)
      void (async () => {
        try {
          const recentResps = await db
    ...
    if (email && process.env.RESEND_API_KEY) {
      try {
        const [dcRow] = await db
          .select({ code: discount_codes.code, expires_at: discount_codes.expires_at })
          .from(discount_codes)
          .where(eq(discount_codes.response_id, response_id))
          .limit(1)

        const [surveyRow] = await db
          .select({
            title: surveys.title,
            discount_value: surveys.discount_value,
            store_id: surveys.store_id,
          })
          .from(surveys)
          .where(eq(surveys.id, id))
          .limit(1)
        ...
        if (dcRow && surveyRow) {
          ...
          const storeName = storeRow?.store_name ?? '餐廳'
          const expiryDate = new Date(dcRow.expires_at).toLocaleDateString('zh-TW')
          const today = new Date().toLocaleDateString('zh-TW', { year: 'numeric', month: '2-digit', day: '2-digit' })
          const benefitLabel = prize_label || surveyRow.discount_value || '專屬優惠'
          ...
          <td style="padding:24px 32px;text-align:center">
            ...
            <div style="display:inline-flex;align-items:center;gap:24px;flex-wrap:wrap;justify-content:center">
              <div style="background:#FFF8F0;border-radius:16px;padding:16px;border:2px solid #FFD4A0">
                <img src="${qrUrl}" width="150" height="150" alt="優惠碼 QR Code" style="display:block;border-radius:8px" />
