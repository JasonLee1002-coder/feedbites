...
export default function MenuPage() {
  ...
  async function fetchCategories() {
    try {
      const res = await fetch(`${BASE_PATH}/api/menu/categories`);
      if (res.ok) {
  ...
  async function handleRenameCategory(id: string, name: string) {
    if (!name.trim()) return;
    try {
      const res = await fetch(`${BASE_PATH}/api/menu/categories/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDeleteCategory(id: string) {
    ...
    setDeletingCatId(id);
    try {
      const res = await fetch(`${BASE_PATH}/api/menu/categories/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  async function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>, dishId?: string) {
    ...
    try {
      ...
      const res = await fetch(`${BASE_PATH}/api/dishes/upload-photo`, {
        method: 'POST',
        body: formData,
      });
      const data = await res.json();
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        // Update
        const res = await fetch(`${BASE_PATH}/api/dishes/${editingId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: formName.trim(),
  ...
  async function handleDelete(id: string) {
    ...
    setDeleting(id);
    try {
      const res = await fetch(`${BASE_PATH}/api/dishes/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  async function handleClearAll() {
    ...
    setClearingAll(true);
    try {
      const res = await fetch(`${BASE_PATH}/api/dishes`, { method: 'DELETE' });
      if (res.ok) {
        setDishes([]);
        resetForm();
      }
  ...
  async function uploadDishPhotoBlob(blob: Blob, dishName: string): Promise<string | null> {
    try {
      ...
      const res = await fetch(`${BASE_PATH}/api/dishes/upload-photo`, { method: 'POST', body: formData });
      if (res.ok) {
        const data = await res.json();
  ...
  async function handleMenuUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      let res: Response;
      try {
        res = await fetch(`${BASE_PATH}/api/ai/parse-menu`, {
          method: 'POST',
          body: formData,
