...
- **網址路徑（2026-09-13 Jason 定）**：basePath 從 `/feedbites` 改成 `/eatagain`，nginx 保留 `/feedbites` 301 轉址與舊上傳圖檔路徑。欣殿萬飲 QR 立牌尚未印，印之前一律用 `/eatagain/s/<surveyId>`
...
## 更上位的總綱（2026-09-15，Jason 指示全體透通）
常來點是「餐飲零售 AI 化」的資料起點，整條線（顧客帳本 → 東方美冷藏櫃 → 通路設備連總部 → 科技店 → 日本旗艦店）與 AI 應用清單、五級框架、定價原則都在全體共用總綱：`~/.claude/shared_intel/knowledge_base/strategy/restaurant_retail_ai_playbook.md`。動任何會改變資料模型或商業前提的功能前先讀它的第四節與第八節；改了設計定案也要回寫那裡，不能只留在本目錄。
